...
(function (Scratch) {
  ...
  class SPrenderControl {
    ...
    getID(args, util) {
      ...
      if (args.TARGET === "_pen_")
        return this.findSpecial(render._allDrawables, "a_lineColorIndex");
    ...
    exportID(args) {
      ...
      const imageData = vm.runtime.renderer.extractDrawableScreenSpace(
        args.ID,
      ).imageData;
    ...
    effectID(args) {
      ...
      if (!ScratchEffects.some((effect) => effect.value === args.EFFECT))
        return;
    ...
    scaleID(args) {
      ...
      const allLay = vm.renderer._drawList;
      if (allLay.indexOf(args.ID) !== -1)
        render._allDrawables[args.ID].updateScale([x, y]);
    }
